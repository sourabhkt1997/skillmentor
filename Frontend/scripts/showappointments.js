...
prevNextIcon.forEach(icon => { // getting prev and next icons
   ...
   submitslot.addEventListener("click",()=>{
    ...
    // Convert month name to its numerical representation
     ...
     var dateTime = new Date(parseInt(yearselected), monthNum, dateselected, hours, minutes);
      ...
      fetch(`${baseurl}slot/createSlot/${userid._id}`,{
        method:"POST",
        headers:{
  ...
  // show slot
    fetch(`${baseurl}slot/allslotTeacher/${userid._id}`)
    .then((res)=>res.json())
    .then((data)=>{
  ...
  function renderslots(data) {
    let slotCardsHTML = data.map((ele) => {
      ...
      const statusClass = ele.status ? 'booked' : 'not-booked';
      ...
      return `
        <div class="slot-card">
          ...
          <div class="${statusClass}">${ele.status ?`<a class="showbookingdetails" href="">Show  booking details</a>` : "Not Booked"}</div>
          </div>
        </div>
      `;
    });
