...
function render(data){  
 ...
 data.forEach((element)=>{
    console.log(element)
      ...
      if(element.uploadedimage){
        image.setAttribute("src",`data:image/jpeg;base64, ${element.uploadedimage}`)
      }
      else{
  ...
  fetch(`${baseurl}user/appointedtutor`)
  .then(res=>res.json())
  .then(data=>{
    console.log(data)
     render(data.msg)
...
checkboxes.forEach(checkbox => {
    checkbox.addEventListener('click', event => {
        ...
        render(data.msg)
          ...
          console.log(valbag)
          fetch(`${baseurl}user/filter?${valbag}`)
         .then(res=>res.json())
          .then(data=>{
          render(data.msg)
          console.log(data)
...
sortselect.addEventListener("change",event=>{
  ...
  if(selected=="low to high"){
   let valbagsort=valbag
    ...
    fetch(`${baseurl}user/filter?${valbagsort}`)
    .then(res=>res.json())
     .then(data=>{
     render(data.msg)
     console.log(data,"88")
