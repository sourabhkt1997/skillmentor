...
authRoute.get("/auth/google/callback", passport.authenticate('google', {
    ...
    if (user) {
      ...
      res.redirect(`https://effulgent-cuchufli-2e38f1.netlify.app?userdata=${encodeURIComponent(JSON.stringify(user))}`)
      // res.redirect(`=${encodeURIComponent(JSON.stringify(user))}`)
    } else {
